...
# Remove the first 50 pariticpants who got bad code
length(unique(d$ID.Ibex))
# Get the first 50 unique values
to_remove <- unique(d$ID.Ibex)[1:49]
# Filter out rows where ID.Ibex is in to_remove
d <- d[!(d$ID.Ibex %in% to_remove), ]
length(unique(d$ID.Ibex))
